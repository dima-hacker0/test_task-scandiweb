...
import { calculateAmountGoodsInBasket } from "../../../functions/functions";
...
class Basket extends React.Component {
  ...
  render() {
    const { basketIsOpened, goods, hideContent } = this.props;
    const amountGoods = calculateAmountGoodsInBasket(goods);
    return (
      <div className={classes.basket}>
        <div className={classes.blockBasketIcon}>
          <Icons className={classes.iconBasket} name="basket" />
          {goods.length !== 0 ? (
            <div className={classes.amountGoods}>{amountGoods}</div>
          ) : null}
        </div>
        <OrderedGoods
